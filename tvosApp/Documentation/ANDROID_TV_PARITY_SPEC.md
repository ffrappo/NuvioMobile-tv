...
Platform-blocked, documented rather than wired: in-app trailer playback (Cinemeta trailers are YouTube ids; tvOS exposes no YouTube playback or launch path to third-party apps), company-logo and comments sections on details (Android sources them from TMDB/Trakt enrichment integrations the tvOS client does not have), Trakt/TMDB-backed post-play recommendations (same-genre Cinemeta stand-in), and physical Apple TV validation (no hardware). These are the only remaining deviations from the Android reference; everything else in the audited scope is wired and tested.
