...
| Timeline seeking | `PlayerControls.kt` | Focused native pan scrubbing with a 700 ms trailing seek debounce and proportional directional fallback |
...
- `PlaybackTimelineScrubber.swift` owns the remote-first timeline. On tvOS 26.6, its focusable UIKit control receives the Siri Remote through `UIPanGestureRecognizer`, following Swiftfin's current tvOS architecture and its 200-point damping on a 0-100 progress scale. A 700 ms trailing debounce groups movement into one exact seek. The preview remains authoritative until MPV confirms that destination, preventing stale playback updates from snapping the playhead back. Directional fallback moves one percent of the title per press, bounded to 15 through 90 seconds, while VoiceOver retains 10-second increments.
...
- Installed AppleTVOS 27.0 headers were checked for deployment availability. `UIPanGestureRecognizer` is available before tvOS 26.6. The interaction follows the maintained Swiftfin tvOS `SliderContainer.swift` pattern reviewed at commit `b2e247f959719044925da4c44fbb7c6e81b42aab` (2026-08-13), including the proven 200-point damping constant. Physical acceptance targets Apple TV HD running tvOS 26.6.
