...
| Timeline seeking | `PlayerControls.kt` | Continuous touch-surface scrubbing with duration-aware accelerated press-and-hold fallback |
...
- `PlaybackTimelineScrubber.swift` owns the remote-first timeline. On tvOS 26.6 it reads the Siri Remote touch surface through the public `GCMicroGamepad` directional input and maps horizontal travel proportionally across the title duration. It previews continuously and issues one exact seek when the finger lifts. Click-pad and keyboard left or right repeats use duration-aware acceleration, while VoiceOver adjustable actions retain predictable 10-second increments.
...
- Installed AppleTVOS 27.0 headers were checked for deployment availability. `GCMicroGamepad`, `dpad.valueChangedHandler`, and `reportsAbsoluteDpadValues` are available before tvOS 26.6. Physical acceptance targets Apple TV HD running tvOS 26.6.
