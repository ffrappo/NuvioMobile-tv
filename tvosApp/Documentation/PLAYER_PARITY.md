...
| Timeline seeking | `PlayerControls.kt` | Focused touch-surface pan scrubbing with predictable 10-second directional and accessibility fallback |
...
- `PlaybackTimelineScrubber.swift` owns the remote-first timeline. On tvOS 26.6, a focused UIKit `UIPanGestureRecognizer` maps horizontal Siri Remote travel against the visible track width. One track-width of travel equals one title duration, with no duration multiplier, acceleration, or absolute touch-position jump. The seek commits when the pan ends. Directional and VoiceOver adjustments use predictable 10-second increments.
...
- Installed AppleTVOS 27.0 headers were checked for deployment availability. `UIPanGestureRecognizer` is available before tvOS 26.6. The interaction follows the maintained Swiftfin tvOS slider pattern reviewed at commit `b2e247f959719044925da4c44fbb7c6e81b42aab` (2026-08-13), while Nuvio omits Swiftfin's optional inertia and speed-boost layers. Physical acceptance targets Apple TV HD running tvOS 26.6.
